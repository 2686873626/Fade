...
class PatternMixer(nn.Module):
    def __init__(self, num_basic=5, num_mixed=4, num_frame=8, num_nodes=5):
        ...
        self.temp_extent = nn.Parameter(torch.ones(num_basic, num_frame*2-1))

        self.pattern_mix = nn.Sequential(
            nn.Linear(num_basic*num_nodes, num_mixed*num_nodes),
            Rearrange('m (h n) -> h n m', h=num_mixed),
            nn.ReLU()
        )

        mixed_mat = torch.ones(
            self.num_mixed,
    ...
    def forward(self, mat):
        ...
        # mat = self.temp_extent(mat)
        temp_extent = F.relu(self.temp_extent)
        mat = einsum('h t, h n m -> h t n m', temp_extent, mat)
        # mask = torch.zeros(4, 7, 7).to(mat.device)
        # mask[:, 6, 6] = 1.0
        # for i in range(15):
        #     if i<=5 or i>=9:
        #         mat[:4, i] = mat[:4, i]*mask

        mat = rearrange(mat, 'h t n m -> (m t) (h n)')
        mat = F.relu(mat)
        ...
        # for i in range(1, 1+clip_len*group_size):
        #     mixed_mat[:, i, 0] = mixed_mat[:, i, 1:].mean(dim=1)
